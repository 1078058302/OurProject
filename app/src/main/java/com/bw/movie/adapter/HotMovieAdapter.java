...
public class HotMovieAdapter extends RecyclerView.Adapter<HotMovieAdapter.MyViewHolder> {
    ...
    @Override
    public void onBindViewHolder(@NonNull HotMovieAdapter.MyViewHolder myViewHolder, final int i) {
        String imageUrl = list.get(i).getImageUrl();
        myViewHolder.simpleDraweeView.setImageURI(imageUrl);
        myViewHolder.movie_name.setText(list.get(i).getName());
        myViewHolder.movie_fragment_item_relative.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                int movie_id = list.get(i).getId();
